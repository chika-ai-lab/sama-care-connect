...
export default function PatientDetail() {
  ...
  const references = mockReferencesSonu.filter(
  ...
  const handleAlertSonu = () => {
    if (risque?.niveau === "rouge") {
      toast.success(
        `Alerte SONU envoyée pour ${patient.prenom} ${patient.nom}`
      );
    }
  };
  ...
  const getStatusBadge = (statut: string) => {
    switch (statut) {
      case "actif":
        return (
          <Badge className="bg-[hsl(var(--status-vert))] text-white">
            Actif
          </Badge>
        );
      case "en_attente":
        return (
          <Badge className="bg-[hsl(var(--status-orange))] text-white">
            En attente
          </Badge>
        );
      case "a_renouveler":
        return (
          <Badge className="bg-[hsl(var(--status-rouge))] text-white">
            À renouveler
          </Badge>
        );
      default:
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-muted-foreground">
              Renouvellement: {patient.date_renouvellement}
            </p>
          </CardContent>
        </Card>
            ...
            <CardContent className="space-y-4">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      {visite.checklist_ok && (
                        <CheckCircle2 className="h-4 w-4 text-[hsl(var(--status-vert))]" />
                      )}
                      {visite.rappel_envoye && (
                        <Bell className="h-4 w-4 text-[hsl(var(--status-orange))]" />
                      )}
                      {visite.statut === "planifie" && (
                        <Clock className="h-4 w-4 text-[hsl(var(--status-orange))]" />
                      )}
                    </div>
                    <Badge
                      className={
                        visite.statut === "realise"
                          ? "bg-[hsl(var(--status-vert))] text-white"
                          : visite.statut === "planifie"
              ...
              {patient.plan_naissance && (
                <div className="p-4 bg-[hsl(var(--status-vert)/0.1)] border border-[hsl(var(--status-vert)/0.3)] rounded-lg">
                  <p className="text-sm font-semibold text-[hsl(var(--status-vert))]">
                    Plan de naissance documenté
                  </p>
              ...
              {visitesCPoN.length > 0 ? (
                visitesCPoN.map((visite) => (
                  <div key={visite.id} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm">
                          ({visite.jours_postpartum} jours post-partum)
                        </span>
                      </div>
                      <Badge
                        className={
                    ...
                    {visite.statut === "rappel_envoye" && (
                      <p className="text-xs text-[hsl(var(--status-orange))] mt-2">
                ...
                <p className="text-center text-muted-foreground py-8">
                  Aucune consultation post-natale prévue
                </p>
              )}
            </CardContent>
          </Card>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {risque.facteurs.map((facteur, idx) => (
                      <div
                        key={idx}
                        className="flex items-start gap-2 p-3 bg-[hsl(var(--status-orange)/0.1)] border border-[hsl(var(--status-orange)/0.3)] rounded"
                      >
                  ...
                  {risque.prediction && (
                    <div className="p-4 bg-[hsl(var(--status-rouge)/0.1)] border border-[hsl(var(--status-rouge)/0.3)] rounded-lg">
                      <p className="text-sm font-semibold text-[hsl(var(--status-rouge))] mb-1">
                        Prédiction IA
                      </p>
                  ...
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className="text-sm text-[hsl(var(--status-orange))]">
                      {risque.niveau === "rouge"
                        ? "🔴 Référence prioritaire vers structure SONU"
                        : risque.niveau === "orange"
                        ? "🟠 Surveillance renforcée - CPN rapprochées"
                        : "🟢 Suivi normal"}
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="mt-4 flex items-center justify-center">
                    <QRCode value={`${patient.qr_code}`} size={120} />
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="p-4 border rounded-lg">
        ...
        <TabsContent value="nutrition">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                {patient.imc && (
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className="text-xs text-[hsl(var(--status-vert))]">
                      Cible atteinte
                    </p>
                  </div>
                )}
              ...
              {patient.hemoglobine && (
                <div className="p-4 border rounded-lg">
                  ...
                  {patient.hemoglobine < 11 && (
                    <p className="text-xs text-[hsl(var(--status-rouge))] mt-1">
                      ⚠️ Anémie détectée
                    </p>
                  )}
                </div>
              ...
              <div className="p-4 bg-[hsl(var(--status-vert)/0.1)] border border-[hsl(var(--status-vert)/0.3)] rounded-lg">
                <p className="text-sm font-semibold text-[hsl(var(--status-vert))] mb-2">
                  Conseils nutritionnels (audio en wolof)
                </p>
                  ...
                  {vaccins.map((vaccin) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Date prévue: {vaccin.date_prevue}
                        </p>
                        {vaccin.date_realisee && (
                          <p className="text-sm text-[hsl(var(--status-vert))]">
                            ✓ Réalisé le {vaccin.date_realisee}
                          </p>
                        )}
                      </div>
                      <Badge
                        className={
                          vaccin.statut === "realise"
                            ? "bg-[hsl(var(--status-vert))] text-white"
                            : vaccin.statut === "retard"
          ...
          <Card>
            ...
            <CardContent>
              {references.length > 0 ? (
                  ...
                  {references.map((ref) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="font-semibold text-[hsl(var(--status-rouge))]">
                            {ref.type_alerte}
                          </p>
                          <p className="text-sm text-muted-foreground">
                            Alerte #{ref.id}
                      ...
                      <div className="bg-muted p-3 rounded text-sm">
                        ...
                        <div className="space-y-1">
                          <p>
                            🔔 Alerte:{" "}
                            {new Date(ref.alerte_heure).toLocaleString("fr-FR")}
                          </p>
                          {ref.transport_heure && (
                            <p>
                              🚗 Transport:{" "}
                              {new Date(ref.transport_heure).toLocaleString(
                                "fr-FR"
                              )}
                            </p>
                          )}
                          {ref.admission_heure && (
                            <p>
                              🏥 Admission:{" "}
                              {new Date(ref.admission_heure).toLocaleString(
                                "fr-FR"
                              )}
                            </p>
                          )}
                          {ref.contre_ref_heure && (
                            <p>
                              ✅ Contre-référence:{" "}
                              {new Date(ref.contre_ref_heure).toLocaleString(
                                "fr-FR"
                              )}
                            </p>
                          )}
                ...
                <p className="text-center text-muted-foreground py-8">
                  Aucune référence SONU enregistrée
                </p>
              )}
            </CardContent>
          </Card>
